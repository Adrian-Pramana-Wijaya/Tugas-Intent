        ...
        when (v.id) {
            ...
            R.id.btn_move_with_data -> {
                val moveWithDataIntent = Intent(this@MainActivity, MoveWithDataActivity::class.java)
                    moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_NAME, "Saya Adrian")
                    moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_AGE, 16)
                    moveWithDataIntent.putExtra(MoveWithDataActivity.EXTRA_EMAIL, "adrian_wijaaya_30rpl@student.smktelkom-mlg.sch.id")
                    startActivity(moveWithDataIntent)
            }
            R.id.btn_dial_number -> {
                val phoneNumber = "081390948993"
                val dialPhoneIntent = Intent(Intent.ACTION_DIAL, Uri.parse("tel:$phoneNumber"))
                startActivity(dialPhoneIntent)
            }
            R.id.btn_move_with_object -> {
                val person = Person("Adrian", "adrian_wijaya_30rpl@student.smktelkom-mlg.sch.id", "malang")
